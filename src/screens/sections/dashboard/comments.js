...
const FunctionToShowComments = ({ comments, token, setLoader }) => {
    ...
    const handleEditDelete = (value) => {
        console.log("handle edit delete..................", value)
        setShowEdit(value.id)
    }
    const handleDeleteComments = async (value) => {
        setLoader(true)
        // console.log("handle delete..................", value.id, token)
    ...
    useEffect(() => {
        console.log("Errrrrrrrrrrrrrrrrrrrrrrrrrrrrr", innerReplies);
    }, [innerReplies])
    const handleInnerReply = async (value) => {
        setLoader(true)
        console.log("Inner ...................reply", value);
        let valueId = value.id;
        await getForumComment(token, valueId).then(data => {
            console.log("React.................", data);
            if (data.error == false && data.errorCode == "") {
                setLoader(false)
            ...
            {comments.replies.map((item, index) => {
                return (
                    <View key={index} style={{ borderWidth: 1, padding: 8, width: "100%", marginBottom: 8, borderRadius: 8, backgroundColor: COLORS.white }}>
                        ...
                        <View style={{ flexDirection: "row", width: "100%", alignItems: "center" }}>
                            ...
                            <View style={{ width: "80%", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                <View style={{ width: "32%", flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                                    <Text style={[styles.textStyle, { fontSize: 10 }]}>{moment(item.createdat).format("DD/MM/YYYY")}</Text>
                        ...
                        {(item.id == showEdit) ?
                            <View style={{ alignSelf: "flex-end", borderRadius: 8, width: "24%", padding: 8, backgroundColor: COLORS.gray }}>
                        ...
                        {innerReplies && (item.id == innerReplies.replies[0].forumid) ? <Text>ccedcedcd</Text> : null}
                    </View >
                )
            })
            }
